...
export class FactoryModelService {
  ...
  create(modelo: string, query: any): Observable<Config> {
    return this._http.post<Config>("http://"+this.url + modelo, query).pipe(
      // retry(3),
      catchError(this.handleError)
    );
  }
  update(modelo: string, referencia: string, query: any): Observable<Config> {
    return this._http.put<Config>("http://"+this.url + modelo + '/' + referencia, query, { headers: this.httpHeaders }).pipe(
      // retry(3),
      catchError(this.handleError)
    );
  }
  delete(modelo: string, referencia: string, query: any) {
    return this._http.delete("http://"+this.url + modelo + '/' + referencia, query).pipe(
      // retry(3),
      catchError(this.handleError)
    );
  }
  get(modelo: string, query: any) {
    ...
    return this._http.get("http://"+this.url + modelo, options).pipe(
      // retry(3),
      catchError(this.handleError)
    );
    // } else {
    //   return this._http.get("http://"+this.url + modelo).pipe(
    //     // retry(3),
    //     catchError(this.handleError)
    //   );
    // }
  ...
  query(modelo: string, query: any) {
    ...
    console.log("http://"+this.url+modelo)
    return this._http.post("http://"+this.url + modelo, query).pipe(
      catchError(this.handleError)
    );
  }
  private adsSecuryty() {
